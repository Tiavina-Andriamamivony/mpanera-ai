...
export const InteractiveBrokerCard = ({
  ...
  return (
    <div
      className={cn(
        "relative flex mx-auto w-full shadow max-w-[1920px] flex-col items-center gap-8 overflow-hidden rounded-lg border bg-card p-8 text-card-foreground md:flex-row md:gap-16 md:p-12",
        className
      )}
    >
      <div className="pointer-events-none absolute right-0 top-1/2 h-[350px] w-[350px] translate-x-1/2 -translate-y-1/2 bg-[radial-gradient(50%_50%_at_50%_50%,hsl(var(--primary)/0.15)_0%,rgba(255,255,255,0)_100%)]" />

      <div className="z-10 flex flex-col items-center text-center md:items-start md:text-left">
        <h2 className="text-3xl font-bold tracking-tight">{name}</h2>
        {announcement ? (
          <p className="mt-2 max-w-xl text-sm text-muted-foreground">{announcement}</p>
        ) : null}
